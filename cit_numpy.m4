...
AC_CACHE_CHECK([for numpy include directory],
    [_cv_numpy_incdir],
    [_cv_numpy_incdir=`$PYTHON -c "import numpy; numpypath=numpy.__path__[[0]]; coredir='core' if numpy.__version__.startswith('1.') else '_core'; print(f'{numpypath}/{coredir}/include')"`])
